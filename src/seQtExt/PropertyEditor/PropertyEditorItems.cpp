...
#include "QDesignerStringListEditorButton.h"
#include "QDesignerLongLongValidator.h"
#include "QDesignerColorButton.h"

#include "QDesignerUtils.h"
...
UnsignedIntProperty::UnsignedIntProperty(unsigned int value, const QString &name)
    : AbstractProperty<unsigned int>(value, name), m_low(0), m_hi(0xffffffff)
{
}

void UnsignedIntProperty::setRange(unsigned int low, unsigned int hi)
{
    m_low = low;
    m_hi = hi;
}

QString UnsignedIntProperty::specialValue() const
{
    return m_specialValue;
}

void UnsignedIntProperty::setSpecialValue(const QString &specialValue)
{
    m_specialValue = specialValue;
}

void UnsignedIntProperty::setValue(const QVariant &value)
{
    m_value = value.toInt();
}

QString UnsignedIntProperty::toString() const
{
    return QString::number(m_value);
}

QWidget *UnsignedIntProperty::createEditor(QWidget *parent, const QObject *target, const char *receiver) const
{
    QSpinBox *spinBox = new QIntPropertySpinBox(parent);
    spinBox->setFrame(0);
    spinBox->setSpecialValueText(m_specialValue);
...
void UnsignedIntProperty::updateEditorContents(QWidget *editor)
{
    if (QSpinBox *spinBox = qobject_cast<QSpinBox*>(editor)) {
        spinBox->setValue(m_value);
    }
...
void UnsignedIntProperty::updateValue(QWidget *editor)
{
    if (QSpinBox *spinBox = qobject_cast<QSpinBox*>(editor)) {
        const int newValue = spinBox->value();
...
OsgVec3fProperty::OsgVec3fProperty(const osg::Vec3f &value, const QString &name)
: AbstractPropertyGroup(name)
{
	DoubleProperty *px = new DoubleProperty(value.x(), QString(QLatin1Char('x')));
	px->setFake(true);
	...
	DoubleProperty *pz = new DoubleProperty(value.z(), QString(QLatin1Char('z')));
	pz->setFake(true);
	pz->setParent(this);

	m_properties << px << py << pz;
}

QVariant OsgVec3fProperty::value() const
{
	QVariant v;
	osg::Vec3f vec(propertyAt(0)->value().toDouble(),
				   propertyAt(1)->value().toDouble(),
				   propertyAt(2)->value().toDouble());
	v.setValue(vec);
	return v;
...
void OsgVec3fProperty::setValue(const QVariant &v)
{
	osg::Vec3f vec = v.value<osg::Vec3f>();
	propertyAt(0)->setValue(vec.x());
	propertyAt(1)->setValue(vec.y());
	propertyAt(2)->setValue(vec.z());
}
...
OsgBoundingSphereProperty::OsgBoundingSphereProperty(const osg::BoundingSphere &value, const QString &name)
: AbstractPropertyGroup(name)
{
	DoubleProperty *x = new DoubleProperty(value.center().x(), QString(QLatin1Char('x')));
	x->setFake(true);
	x->setParent(this);

	DoubleProperty *y = new DoubleProperty(value.center().y(), QString(QLatin1Char('y')));
	y->setFake(true);
	y->setParent(this);

	DoubleProperty *z = new DoubleProperty(value.center().z(), QString(QLatin1Char('z')));
	z->setFake(true);
	z->setParent(this);

	DoubleProperty *radius = new DoubleProperty(value.radius(), "Radius");
	radius->setFake(true);
	radius->setParent(this);

	m_properties << x << y << z << radius;
}

QVariant OsgBoundingSphereProperty::value() const
{
	QVariant v;
	osg::Vec3 center(propertyAt(0)->value().toDouble(),
					 propertyAt(1)->value().toDouble(),
					 propertyAt(2)->value().toDouble());
	double radius = propertyAt(3)->value().toDouble();
...
void OsgBoundingSphereProperty::setValue(const QVariant &v)
{
